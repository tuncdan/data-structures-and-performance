...
public class TextProController {
	...
	private void initialize() {
		...
		src.spelling.Dictionary dic = launch.getDictionary();
	...
	private void handleFleschIndex() {
		...
		if(!text.equals("")) {
			...
			src.document.Document doc = launch.getDocument(text);
	...
	private void handleMarkovText() {
		// get MTG object
		src.textgen.MarkovTextGenerator mtg = launch.getMTG();
		
		Task<src.textgen.MarkovTextGenerator> task = new Task<src.textgen.MarkovTextGenerator>() {
	        @Override
	        public src.textgen.MarkovTextGenerator call() {
	    ...
	    task.setOnSucceeded(e -> {
	    	loadStage.close();
			src.textgen.MarkovTextGenerator result = task.getValue();
